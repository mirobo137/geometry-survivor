...
try {
  ...
  assert.equal(await page.locator('article').count(), 17);
  assert.equal(await page.locator('article svg').count(), 51);
  ...
  console.log(`Fleet gallery verified: 17 assets, unique IDs, four inspection modes, eight backgrounds. Captures: ${output}`);
} finally {
  await browser.close();
}
