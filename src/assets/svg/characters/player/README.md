...
`core` propios: cyan vive junto al master; violet, amber, emerald, obsidian y nova están en
...
`?skin=amber`, `?skin=emerald`, `?skin=obsidian` o `?skin=nova` no cambian colisión ni stats.
...
`?cannon=rainbow`, `?cannon=lattice` o `?cannon=helix` permiten revisar las variantes en una run.
